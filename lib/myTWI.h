...
#define STOP_Rec				0xA0 //STOP condition received
...
volatile uint8_t SLAVE_wData[100];	//buffer for Slave transmitter 
volatile uint8_t Rec_Num=0;			//number of received data byte
volatile uint8_t SLAVE_mode=0; 		//0: own address mode, 1: General call
volatile uint8_t Device_Addr=2;		//the address in SLAVE mode
volatile uint8_t SLAVE_buff[100]; 	//SLAVE_buff: received data
//END FOR SLAVE USE************************
